...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0 <= sigmoid(-2) <= 1
    assert 0 <= sigmoid(0) <= 1
    assert 0 <= sigmoid(2) <= 1
    assert 0 <= sigmoid(100) <= 1

    assert_close(sigmoid(-2) + sigmoid(2), 1.0)
    assert_close(sigmoid(-1) + sigmoid(1), 1.0)
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    for x in arr:
        for y in arr:
            for z in arr:
                if x < y < z:
                    assert lt(x, y) == 1.0 and lt(y, z) == 1.0 and lt(x, z) == 1.0
...
def test_other() -> None:
    ...
    for i in range(-10, 10):
        for j in range(-10, 10):
            for k in range(-10, 10):
                assert_close(mul(i, mul(j, k)), mul(mul(i, j), k))
                assert_close(add(i, add(j, k)), add(add(i, j), k))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for x in ls2:
        res = add(res, x)
    assert_close(add(sum(ls1), sum(ls2)), res)
